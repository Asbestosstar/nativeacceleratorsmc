...
final class MetalSurface implements GpuSurfaceBackend {
    ...
    MetalSurface(MetalDevice device,long window,BooleanSupplier iconified) {
        ...
        if (MetalTrace.enabled()) MetalTrace.log("SURFACE_CREATE", "window=" + MetalTrace.hex(window) + " device=" + MetalTrace.hex(device.handle()));
    ...
    @Override public void configure(GpuSurface.Configuration config) throws SurfaceException {
        this.config=config;
        if (MetalTrace.enabled()) MetalTrace.log("SURFACE_CONFIGURE", "window=" + MetalTrace.hex(window) + " size=" + config.width() + "x" + config.height() + " mode=" + config.presentMode());
    ...
    @Override public void acquireNextTexture() throws SurfaceException { if(config==null)throw new SurfaceException("Metal surface is not configured");acquired=true; if (MetalTrace.enabled()) MetalTrace.log("SURFACE_ACQUIRE_REQUEST", "nextFrame=" + (presentSequence+1) + " configured=" + config.width() + "x" + config.height()); }
    @Override public void blitFromTexture(CommandEncoderBackend encoderBackend,GpuTextureView source) {
        ...
        try(MemoryStack stack=MemoryStack.stackPush()){
            long frame=++presentSequence; if (MetalTrace.enabled()) { MetalTrace.frame(frame); MetalTrace.log("FRAME_BEGIN", "frame=" + frame + " source=" + MetalTrace.hex(view.metalTexture().handle()) + " sourceSize=" + view.getWidth(0) + "x" + view.getHeight(0)); }
            if(device.waitForSwapchainBeforeAcquire()){
                long waitNs=System.nanoTime(); Object waited=MetalInterop.sdlCall("SDL_WaitForGPUSwapchain",device.handle(),window); if (MetalTrace.enabled()) MetalTrace.log("SWAPCHAIN_WAIT", "frame=" + frame + " ns=" + (System.nanoTime()-waitNs) + " result=" + waited);
            ...
            if (MetalTrace.enabled()) MetalTrace.log("SWAPCHAIN_ACQUIRE_BEGIN", "frame=" + frame + " encoder=" + encoder.traceEncoderId() + " cmd=" + MetalTrace.hex(encoder.commandHandle()));
            ...
            long swap=texture.get(0); if (MetalTrace.enabled()) MetalTrace.log("SWAPCHAIN_ACQUIRE_END", "frame=" + frame + " result=" + ok + " texture=" + MetalTrace.hex(swap) + " size=" + w.get(0) + "x" + h.get(0) + " ns=" + (MetalPerfCounters.tic()-waitStart));
            if(swap==0L){
                if (MetalTrace.enabled()) MetalTrace.log("SWAPCHAIN_EMPTY", "frame=" + frame);
                return;
            }
    ...
    @Override public void present(){ acquired=false; MetalPerfCounters.present(); if (MetalTrace.enabled()) MetalTrace.log("SURFACE_PRESENT_MARK", "frame=" + presentSequence); /* SDL presents automatically when the acquiring command buffer is submitted. */ }
